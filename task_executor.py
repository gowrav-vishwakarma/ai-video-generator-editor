...
class TaskExecutor:
    ...
    def _execute_generate_audio(self, scene_idx: int, text: str, speaker_wav: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if actual_audio_duration <= 0.1:
                ...
                self.project_manager.update_narration_part_status(scene_idx, "generated", scene_audio_path, 0.0)
                return True
            
            self.project_manager.update_narration_part_status(
                scene_idx, "generated", scene_audio_path, actual_audio_duration
            )
            return True
        except Exception as e:
    ...
    def _execute_create_scene(self, scene_idx: int) -> bool:
        ...
        try:
            ...
            chunk_specific_prompts = self.llm_module.generate_chunk_visual_prompts(
                narration["text"], visual_prompt_data["prompt"], num_video_chunks, self.content_cfg, self.llm_cfg
            )
            self.llm_module.clear_llm_vram()
    ...
    def _execute_generate_chunk(self, scene_idx: int, chunk_idx: int, visual_prompt: str, motion_prompt: Optional[str] = None) -> bool:
        ...
        try:
            ...
            chunk = scene['chunks'][chunk_idx] if scene else None
            
            if not scene or not chunk:
                logger.error(f"Could not find scene {scene_idx} or chunk {chunk_idx}")
                return False
    ...
    def _execute_assemble_scene(self, scene_idx: int) -> bool:
        ...
        try:
            ...
            if not scene:
                logger.error(f"Could not find scene {scene_idx}")
                return False
                
            video_sub_clip_paths = [c["video_path"] for c in scene["chunks"] 
                                  if c["status"] == "video_generated"]
            
            if len(video_sub_clip_paths) != len(scene['chunks']):
                logger.error(f"Cannot assemble scene {scene_idx}, not all chunks are generated.")
                return False
        ...
        try:
            ...
            for scene in self.project_manager.state.scenes:
                if scene["status"] == "completed":
                    # --- FIX IS HERE ---
                    # We are now re-adding the narration_data dictionary to the tuple.
                    # This will create a 3-item tuple, which assemble_final_reel expects.
                    narration_data = {
            ...
            if len(processed_scene_assets) != len(self.project_manager.state.scenes):
                logger.error("Cannot assemble final video, not all scenes are completed.")
                return False
            ...
            if final_video_path and os.path.exists(final_video_path):
                # --- FIX IS HERE ---
                # It's more robust to build the text from the assets we just collected.
